...
# home location for these is in $PELETEST_HOME/job_scripts.
...
if [ -d "job_scripts" ]; then
  ...
  script_dir="$PELETEST_HOME/job_scripts"
fi
...
function copy_files {
  ...
  if [ ! -e "$dir/job_scripts/run_utils.sh" ] && [ -z "$inputs_only" ]; then
      ...
      cp -r $PELETEST_HOME/job_scripts/*.sh $dir/job_scripts/
...
function create_job_script {
  ...
  if [ $batch_system == "PBS" ]; then
	...
	echo "" >> $dir/$job_script
	  ...
	  echo "PELETEST_HOME=$PELETEST_HOME" >> $dir/output/$archive_script
